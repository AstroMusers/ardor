...
def SPI_Geo(period, e, a, Rst, arg_peri, los_factor):
    Rst = Rst*0.00465047
    lower,upper = OML.secondary_eclipse_true_anomaly_range_exact(a, e, 90, 180 + arg_peri, Rst, period)
    lower = U.range_shift(lower, 0, 2*np.pi, 0, 360)
    upper = U.range_shift(upper, 0, 2*np.pi, 0, 360)
    phase, dist, rot = OML.orbit_pos_v_time(period, e, a, phase = True, arg_periastron= 180, orbit_length=500)
    norm_orbit = (dist)**-3
...
def SPI_Geo_flare_injection(light_curve, period, a, e, Rst, arg_periastron, los_factor, sp_type='G',
                              flare_type = 'Flaring', fast = False):
    location_list = []
    ...
    length = len(lc.time)
    model, b = SPI_Geo(period, e, a, Rst, arg_periastron, los_factor)
    ## Assume phase is random to begin each light curve. Periastron at 0.5
    random_time = lc.time[int(len(lc.time)/2)]
    phase_array = ((lc.time - (random_time+ period/2)) % period)/period
    ...
    for interval in range(length - 200):
        ...
        flare_rate = model[find_nearest(b, phase)]*rate
