...
targets = pd.read_csv("C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/KS_KU_Tests/K_Tests/Exo/K_All_Exo.csv")
targets_TOI = pd.read_csv("C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/KS_KU_Tests/K_Tests/TOI/K_All_TOI.csv")
...
directory = os.listdir('C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Flare_Catalogs/All TOIs/TOIs')

targets.sort_values(by='p_KU', inplace=True, ascending = False)
targets_TOI.sort_values(by='p_KU', inplace=True, ascending = False)
...
#         elif peri == True:
#             continue
#         files = os.listdir("C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Flare_Catalogs/Exoplanet_Hosts/Hosts/" + str(stars))
...
#         for data in files:
#             try:
#                 data_dir = "C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Flare_Catalogs/Exoplanet_Hosts/Hosts/" + str(stars) + '/' + str(data)
#                 lc = tier0(data_dir)
...
for index, star in enumerate(hosts_TOI):
    if star in hosts_TOI:
        try:
            ...
            flare_epochs = np.array(flares_TOI.loc[flares_TOI['Host_ID'] == float(stars), 'Flare_Epoch_TESS'])
            ...
            # peri_lower = peri_lower[0]
            dlogZ = np.array(targets_TOI.loc[targets_TOI['Host_ID'] == float(stars), 'dlog(Z)'])
            dlogZ = dlogZ.item()
            KS = np.array(targets_TOI.loc[targets_TOI['Host_ID'] == float(stars), 'p_KS'])[0]
            KS = KS.item()
            KU = np.array(targets_TOI.loc[targets_TOI['Host_ID'] == float(stars), 'p_KU'])[0]
            KU = KU.item()
            AD = np.array(targets_TOI.loc[targets_TOI['Host_ID'] == float(stars), 'p_AD'])[0]
            AD = AD.item()
            K = [KS, KU, AD]
            K_val = np.argmin(np.array(K))
            K_str = ['KS', 'KU', 'AD']
            if (K[K_val] < 5e-2 and dlogZ > 5) and len(flare_epochs) >7:
                if peri == False:
                    try:
                        flare_epochs2 = np.array(flares_TOI.loc[flares_TOI['Host_ID'] == int(stars), 'Flare_Epoch'])
                    except:
                    ...
                    continue
                files = os.listdir('C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Flare_Catalogs/All TOIs/TOIs/' + str(stars))
                time = np.linspace(1250, 4000, num=100000)
                # phases = np.mod((np.linspace(2457000 + 1250, 2457000 + 4000, num=100000) - (periastron_epoch + period/2)), period)/period
                check = 0        
                for data in files:
                    try:
                        data_dir = 'C:/Users/whitsett.n/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Flare_Catalogs/All TOIs/TOIs/' + str(stars) + '/' + str(data)
                        lc = tier0(data_dir)
                        if np.min(time) < min_time:
                            min_time = np.min(time)
                        if np.max(time) > max_time:
                ...
                # ax.text(3500, .9625 + shift,str(proper[index]) +'\n' + str(len(flare_epochs)) +' Flares' + '\n' + 'KU: ' + str("{:.2E}".format(round(KU, 3))) + ' KS: ' + str("{:.2E}".format(round(KS, 4))), font=font,fontsize = 12)
                ax.text(3600, .954 + shift,str(star) +'\n$N_{flare} = ' + str(len(flare_epochs)) + '$\n$p_{' + K_str[K_val] + '}:\,' + str(round(K[K_val], 3)) + '$\n' + "$\overline{\Delta\log(Z)}=$" + str(round(dlogZ,1)) , font=font,fontsize = 9, horizontalalignment='left')
