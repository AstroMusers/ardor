...
def FFD_Dict(host_names, energies, periods, observation_times, Teffs = None, dlogZs = None, metallicities = None):
    for keys in host_names:
        FFD_Dict = {keys:(energies, periods, observation_times, Teffs, dlogZs, metallicities)}
    return FFD_Dict



def FFD_Generator(data_dir):
    for host in hosts:
        if host != host_:
            energy_count_list = []
            energy_count_list2 = []
