...
def K_Test_Sim(DataFrame, output_dir):
    KS_Sample_list = []
    AD_Sample_list = []
    N = 50

    interest_hosts = [['Host_ID', 'p_KU', 'p_KS', 'p_AD', 'p_KS_Samp', 'p_AD_Samp', 'N']]
    ...
    for index, hosts in enumerate(set_hosts):
        print(index)
        phases = np.array(DataFrame.loc[DataFrame['Host_ID'] == hosts, 'Phase'], dtype=float)
        # obs_time = 700300
        peri_phases = np.array(DataFrame.loc[DataFrame['Host_ID'] == hosts, 'Phase'])
        # peri_phases_lower = np.abs(np.array(DataFrame.loc[DataFrame['Host_ID'] == hosts, 'Periastron_Lower'])[0])
        # peri_phases_upper = np.array(DataFrame.loc[DataFrame['Host_ID'] == hosts, 'Periastron_Upper'])[0]
        # observation_time = np.array(TOI_flares.loc[TOI_flares['Host_ID'] == hosts, 'Observation_Time'])[0]
        # energy = np.array(DataFrame.loc[DataFrame['Host_ID'] == hosts, 'Energy'])
        ...
        for phase_sample in range(N):
            ...
            if check > 0.5:
                peri_phases = peri_phases + np.abs(np.random.normal(0, scale=np.abs(0.1)))
            elif check < 0.5:
                peri_phases = peri_phases - np.abs(np.random.normal(0, scale=np.abs(0.1)))
        ...
        if p_KU < 1:
            interest_hosts.append([hosts, p_KU, p_KS, p_AD,np.mean(KS_Sample_list), np.mean(AD_Sample_list), len(peri_phases)])
...
Kappa_G = pd.read_csv('C:/Users/natha/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Python Scripts/ardor/src/ardor/SPI_Forward_Models/Sim_Outputs_Kappa/G_Type/Kappa_Sim_G.csv')

K_Test_Sim(Kappa_G, 'C:/Users/natha/OneDrive - Washington University in St. Louis/Desktop/Research/Induced_Flares/Python Scripts/ardor/src/ardor/SPI_Forward_Models/Sim_Outputs_Kappa/G_Type/Kappa_Sim_G_K.csv')
# # sigma = 0.05
# num = str(0.5)
